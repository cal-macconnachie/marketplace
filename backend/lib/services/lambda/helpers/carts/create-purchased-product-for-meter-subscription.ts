...
export const createPurchasedProductForMeterSubscription = async ({
  purchaseKey,
  subscriptionItem
}: {
  purchaseKey: {
  ...
  // For metered subscriptions, create purchased product with zero-dollar amount
  // The actual amount will be calculated when the invoice is finalized
  const purchasedProduct: PurchasedProduct = {
    id: v4(),
    product_id: purchase.product_id,
    group_id: purchase.product_group_id,
    name: purchase.product_name,
    amount: 0, // Zero-dollar for metered subscriptions initially
